  ...
  <svg viewBox="0 0 200 200" className={className} xmlns="http://www.w3.org/2000/svg">
    ...
    <g transform="translate(10, 10) scale(0.9)">
      ...
      <g filter="url(#eye-glow)">
        <path d="M60 70 L90 80 L70 90 Z" fill="white" className="drop-shadow-[0_0_15px_rgba(255,255,255,1)]" />
        <path d="M140 70 L110 80 L130 90 Z" fill="white" className="drop-shadow-[0_0_15px_rgba(255,255,255,1)]" />
      </g>
...
const SplashScreen: React.FC<Props> = ({ onEnter, isLoading, error, isMuted, onToggleMute }) => {
  ...
  if (viewState === 'LOADING') {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black overflow-hidden transition-opacity duration-500" style={{ opacity }}>
        <DynamicBackground />
        <div className="relative z-10 flex flex-col items-center justify-center h-full px-4">
           <div className="relative w-48 h-48 md:w-96 md:h-96 animate-[bounce-short_2s_infinite]">
             ...
             <img src="https://i.postimg.cc/KYDcXcDj/image.png" alt="Logo" className="relative z-10 w-full h-full object-contain drop-shadow-[0_0_30px_rgba(0,0,0,0.8)] animate-[shake_4s_ease-in-out_infinite]" />
           </div>
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black overflow-hidden transition-opacity duration-500" style={{ opacity }}>
        ...
        <div className="relative z-10 flex flex-col items-center px-4">
            <p className="text-slate-500 text-xs md:text-sm uppercase tracking-[0.3em] font-bold mb-8 animate-fade-in">Developed By</p>
            <div className="relative group">
                ...
                <img src="https://i.postimg.cc/dtQy51rp/DG-logo.png" alt="DG Logo" className="w-40 md:w-64 object-contain drop-shadow-2xl animate-[scale-in_1s_ease-out]" />
            </div>
        </div>
      </div>
    );
  ...
  if (viewState === 'INTRO_SEQUENCE') {
    const messages = ["WELCOME TO THE STUDIO!", "THIS IS GOING TO BE INSANE!", "I HOPE YOU STUDIED...", "LET'S START THE GAME!"];
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-beast-navy overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center animate-scale-in px-4">
            <div className="relative bg-white text-black p-6 md:p-8 rounded-3xl rounded-bl-none mb-4 md:mb-8 max-w-sm md:max-w-xl text-center shadow-[0_0_40px_rgba(255,255,255,0.4)] animate-[bounce_1s_infinite]">
                 <h2 className="text-2xl md:text-5xl font-black italic uppercase tracking-tighter leading-tight">{messages[introTextIndex]}</h2>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full p-4 text-center overflow-y-auto custom-scrollbar relative transition-opacity duration-1000" style={{ opacity }}>
      <DynamicBackground />

      <button onClick={() => setShowSettings(true)} className="absolute top-6 right-6 z-40 p-3 bg-slate-800/80 hover:bg-slate-700 rounded-full border border-slate-600 transition-all hover:scale-110 shadow-lg group">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center w-full max-w-6xl gap-8 lg:gap-16 z-10 mt-12 md:mt-0">
          ...
          <div className="relative group transform transition-transform hover:scale-105 duration-300">
            <div className="flex flex-col items-center justify-center">
                <div className="flex items-center justify-center relative">
                    ...
                    <h1 className="text-5xl md:text-8xl font-black italic tracking-tighter transform rotate-6 translate-y-4 md:translate-y-8 z-10 ml-[-15px] md:ml-[-20px] bg-gradient-to-r from-beast-blue via-beast-pink to-beast-blue bg-[length:200%_auto] text-transparent bg-clip-text animate-gradient-x drop-shadow-sm text-shadow-lg">BEAST</h1>
                </div>
                
                {/* Updated to PINK as requested */}
                <h2 className="mt-6 md:mt-8 text-xl md:text-3xl font-black text-white bg-black/50 px-6 py-2 rounded-full border-2 border-beast-pink shadow-[0_0_15px_rgba(236,72,153,0.5)] uppercase tracking-[0.2em] transform -skew-x-12">
                  Trivia Challenge
                </h2>
            </div>
          </div>
          ...
          <div className="w-full max-w-sm md:max-w-md animate-fade-in flex flex-col gap-6">
            {error && (
              <div className="text-red-100 bg-red-600/20 p-4 rounded-xl font-bold border border-red-500 backdrop-blur-md">{error}</div>
            )}
            ...
            <button 
               ...
               className="text-beast-pink text-xs md:text-sm font-black uppercase tracking-widest animate-bounce hover:scale-110 transition-transform flex items-center justify-center gap-2 bg-black/30 px-4 py-3 rounded-full border border-beast-pink/30 hover:border-beast-pink hover:bg-black/50"
             >
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-slate-900 border-4 border-slate-700 w-full max-w-sm rounded-3xl p-6 shadow-2xl relative">
            <button onClick={() => setShowSettings(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors"><X size={24} /></button>
            <h2 className="text-2xl font-black uppercase text-white mb-8 flex items-center gap-2"><Settings className="text-beast-blue" /> Settings</h2>
            <div className="bg-slate-800 p-4 rounded-xl flex items-center justify-between">
                ...
                <button onClick={() => { audioService.play('click'); onToggleMute(); }} className={`w-14 h-8 rounded-full p-1 transition-colors ${!isMuted ? 'bg-green-500' : 'bg-slate-600'}`}>
            ...
            <div className="mt-8 text-center text-xs text-slate-500 font-bold uppercase tracking-widest">Version 1.0.0 • Chaos Edition</div>
          </div>
        </div>
      )}
      ...
      {showDonate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-scale-in">
          <div className="bg-gradient-to-br from-slate-900 to-beast-navy border-4 border-beast-pink w-full max-w-md rounded-3xl p-8 shadow-[0_0_50px_rgba(236,72,153,0.2)] relative text-center">
            <button onClick={() => setShowDonate(false)} className="absolute top-4 right-4 bg-black/20 hover:bg-black/40 text-white p-2 rounded-full transition-colors"><X size={24} /></button>
            <div className="w-20 h-20 bg-beast-pink rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl animate-bounce-short">
                <Heart size={40} className="text-white" fill="currentColor" />
            </div>
            <h2 className="text-3xl font-black uppercase text-white mb-4 italic tracking-tight">Fuel the Madness!</h2>
            <p className="text-slate-300 font-medium text-lg mb-8 leading-relaxed">Your support helps us build <span className="text-beast-blue font-bold">wilder games</span>!</p>
            <a href="https://ko-fi.com/digitalghost8" target="_blank" rel="noopener noreferrer" className="block w-full bg-beast-pink hover:bg-pink-500 text-white font-black text-xl py-4 rounded-xl shadow-lg hover:scale-105 transition-all uppercase tracking-widest flex items-center justify-center gap-2 group">
                <ExternalLink size={24} className="group-hover:rotate-45 transition-transform" /> Donate on Ko-fi
            </a>
          </div>
        </div>
      )}
